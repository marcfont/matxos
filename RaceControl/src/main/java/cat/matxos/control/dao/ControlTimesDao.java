...
public interface ControlTimesDao extends JpaRepository<Read, Read.ReadKey> {
    ...
    @Query(value = "select control, count(*) as count from time_reads where race=?1 and time!=-1 group by control", nativeQuery = true)
    String[] getTimesCount(String race);

    @Query(value = "select * from time_reads where race=?1 and time=-1 and control=?2", nativeQuery = true)
    List<Read> getOutsControl(String race, String control);

    @Query(value = "select * from time_reads where race=?1 and time!=-1 and control=?2", nativeQuery = true)
    List<Read> getReadsControl(String race, String control);

    @Query(value = "select * from time_reads where race=?1  and time!=-1 and control in (?2) group by bib", nativeQuery = true)
